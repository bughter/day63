let vm = new Vue({
        ...
        methods: {
            ...
            doUpdate: function () {
                ...
                let zTree = $.fn.zTree.getZTreeObj("select-treetreeSelectResEdit");
                let changedNodes = zTree.getChangeCheckedNodes();
                for (let i = 0; i < changedNodes.length; i++) {
                        if (changedNodes[i].checked = true && changedNodes[i].id!=0) {
                            resources.push(changedNodes[i].id);
                        }
                }
                console.log(resources);
                // axios({
                ...
                //     method:'post',
                //     data:{ 'role': this.role,'redIds':this.resources,'oids':this.offices,'rid':this.role.id}
                // }).then(response => {
                //
                // }).catch(function (error) {
                //     layer.msg(error.message);
            ...
            clickNode: function (event, treeId, treeNode) {
                ...
                this.role.officeName = treeNode.name;
            },
